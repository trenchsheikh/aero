...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      store.getProfileByWallet(wallet),
      store.getMyPrivacyLinks(wallet),
      store.getPaymentsReceived(wallet),
      getBalance(wallet),
    ]).then(([p, links, pays, bal]) => {
      setProfile(p);
  ...
  async function generatePrivacyLink() {
    if (!publicKey) return;
    ...
    await store.savePrivacyLink(link);
    const links = await store.getMyPrivacyLinks(publicKey.toBase58());
    setPrivacyLinks(links);
  }
    ...
    <div className="max-w-2xl mx-auto px-4 py-6 sm:py-10 space-y-5 sm:space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-brand/70 text-sm mt-0.5 font-light">useaero.io/{profile.username}</p>
        </div>
